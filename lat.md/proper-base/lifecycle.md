...
For the thinking level, persistence is what makes the choice hold rather than merely what makes it reappear. Pi recomputes the level inside every `setModel` from `modelThinkingLevels`, then `defaultThinkingLevel`, and only then the level in effect, so the saved default outranks the running session: without a write, a level chosen through `/thinking` or Shift+Tab is discarded by the next `/model`, `Ctrl+P`, `/clear` restore, or routed prompt rather than surviving until the next session. Writing the key the resolver reads makes every later switch resolve back to the same level.

The recorded level is the one Pi emits after clamping to the active model, so moving to a model that cannot reach the current level records the level actually in effect, and moving on from it records whatever is chosen next. The single exception is a level equal to the `modelThinkingLevels` entry for the current model: that rule is per model and already outranks the default in Pi's resolver, so promoting it would hand one model's level to every model without a rule of its own.

Both handlers are turned off together by `"stickyDefaults": false` in the agent directory's `proper-base.json`, the same file the session action rail's choice lives in; a missing or damaged config reads as enabled.
...
The model is set first and the level second. Every `setModel` recomputes the level from `modelThinkingLevels`, then `defaultThinkingLevel`, and only then the level in effect, so a level applied first would be discarded by the switch that follows it. Both calls emit the selection events `Sticky startup defaults` listens for, so a model and level chosen this way persist exactly like ones chosen through Pi's own pickers. Because the command never reaches Pi, the extension reports the result itself; a model that cannot be activated reports the failure and leaves the level alone.
