...
Pi 0.86.0's public `ExtensionAPI` exposes only session-local `setModel(model)` and `setThinkingLevel(level)`. The version-matched `AgentSession` methods already accept `{ persist: true }` and update the live `SettingsManager`, but the extension bridge deliberately omits that option. `src/sticky-defaults.ts` is therefore a narrow compatibility adapter: before `AgentSession` is constructed it ownership-wraps the version-matched mutation methods and runtime builder, activates the discovered live session only at `session_start`, and uses that session's public `SettingsManager` setters. It never mutates cache fields or rewrites `settings.json`. Shutdown deactivates retained wrappers before restoring only methods it still owns, so a foreign wrapper cannot keep persistence active. Reload takes over an earlier proper-base controller. Delete the adapter when ExtensionAPI forwards the host's persistence option.
