...
## History recall fixture

A real pi-tui `Editor` proves Up enters history only from an empty prompt.

With `draft` in the editor, Home then Up leaves the text and the cursor at line 0, column 0 untouched, which is exactly the sequence Pi's own column-0 rule would have turned into a recall. A two-line draft walks Up through its rows, jumps to the line start on the top row, and still never recalls; a single space counts as a draft. Clearing the prompt restores Pi's recall: Up loads the newest entry at line 0, column 0, Up again walks older, and Down returns to the empty draft.
...
An inline-command fixture verifies slash completion receives only the active command segment after whitespace or on a later line, replaces only that segment, supports command arguments, and ignores URL or path-internal slashes. Its acceptance cases prove Tab-accepting `/mo` into `/model ` reopens suggestions once so the argument menu shows without another keystroke, an Esc dismissal that changes no text stays closed, and an accepted multi-segment file path opens nothing. Its stale-list cases prove a word delete that changes the prompt under a still-open list re-requests suggestions exactly once, while plain backspace — which pi refreshes natively — and text-preserving keys request nothing. A model-completion fixture verifies the empty `/model ` list sorts descending, `opu` removes unrelated candidates and sorts Opus versions descending, `opus 4` requires both terms, and non-model suggestions preserve provider order. Its thinking-argument cases prove a completed `provider/id` reference followed by a space lists all seven levels even under a forced request with the level in effect leading, a partial word narrows them, and a second word naming no level keeps sorting models. Its submission cases prove Enter accepts and submits a selected model, Tab accepts the model and submits nothing while appending the separator and requesting the level menu exactly once, a hand-typed separator requests it again while a space outside a `/model` command requests nothing, Enter on a level submits both arguments as one model-and-level change instead of handing Pi a two-word search, Tab on a level submits the same way because nothing can follow it, and a reference matching no model is passed through to Pi untouched.
...
## Prompt mouse fixture

A real pi-tui `Editor` verifies the guard against Pi's own mouse handler rather than a fake.

With the setting off, a left click on the prompt's text row is reported handled while the cursor stays at the end of the draft; a click on the border row keeps Pi's native focus result, and a press on the text row stays unhandled so drag-to-select still reaches the renderer. Re-enabling hands the same click back to Pi, which moves the cursor to the clicked column. Disposal removes the instance override so the prototype handler is live again, and a stale disposer is a no-op. A second case proves repeated installation takes over instead of stacking and that an editor without `handleMouse` installs nothing.
