...
proper-base has no install-time scripts. Runtime sticky defaults use Pi's live `SettingsManager`; `"stickyDefaults": false` disables those writes.
...
Kitty previews use the declared `sharp` 0.35.4 runtime dependency for asynchronous metadata and source loading at every size, resizing sources that exceed the 24-by-6-cell pixel envelope. `sharp` requires Node 20.9+ and publishes prebuilt binaries for macOS arm64, macOS x64 10.15+, and supported glibc/musl Linux targets; proper-base's Node 22.19+ floor satisfies its runtime contract. The package lock retains both Darwin architecture artifacts plus their matching libvips packages, so npm selects the host artifact during production installation. Metadata probing, source loading, and conversion run asynchronously with five-second pipeline timeouts; the editor callback only registers the marker and cancellable work. Failure leaves the safe marker-and-path text fallback instead of sending the full source image.

The extension uses `CustomEditor`, `SessionManager`, `getAgentDir`, session lifecycle events, editor factory APIs, pi-tui overlays and wrapping, and ANSI truecolor footer decoration. The Pi host supplies both imported core packages through `peerDependencies` with `*` ranges, while package-local dev dependencies follow latest Pi and pi-tui, with resolved versions recorded in the lockfile; TypeScript and Node declarations remain pinned for diagnostics. Pi 0.85.0 shipped its experimental server and client code in the package root, so a bare `import "@earendil-works/pi-coding-agent"` in a fresh checkout failed on the undeclared `@earendil-works/pi-server` and needed an extra dev pin (upstream issue 9132); Pi 0.85.1 made that code source-only, so the pin is gone and the SDK entry loads on its declared dependencies alone. Tests use Node's built-in runner and exercise Pi 0.87.0.
