...
<!-- lat-index
- [[proper-compact]]: repository index entry
...
`.release-me.json` selects npm mode, requires releases from `main`, and maps `proper-base`, `proper-compact`, `proper-llm-router`, `proper-pacify`, and `proper-flow` to their tracked manifests. The shared `tools/release-me/release.sh` requires a configured package argument, derives the version from that manifest, and scopes release notes to the package directory. When `pi` is installed, notes generation always runs through `pi` and `--ai` selects the model (codex/auto → `gpt-5.6-sol`, claude → `claude-opus-4-7`) instead of the harness; without `pi` it falls back to the `codex` or `claude` CLIs.

`bump auto` takes no package argument: it iterates every configured package, mechanically skips packages with no commits since their last tag, and for each changed package makes one AI pass that decides the semver level and writes the release notes together. The pass receives the commit messages plus the actual diff, size-capped with lock files excluded, and is instructed to treat the diff as the source of truth so changes that commits omit or under-describe still count. Every decided release is confirmed at a prompt that accepts it, declines that package, or overrides the bump level to major, minor, or patch — manual `bump` confirmations offer the same overrides — and accepted packages release sequentially through the normal publish path, so an interrupted or partly declined run can simply be rerun. Outside npm mode the same flow releases the repository once.
...
All five configured packages must trust the `publish-npm.yml` workflow in `sharaf-nassar/proper-pi-extensions` with environment `npm-release`. A brand-new package receives one maintainer-authenticated initial publish because npm cannot configure trust before the package exists, and its trusted publisher must then be registered before the workflow can publish it; `proper-pacify` has its initial publish but not yet that trust registration. `proper-compact` is not yet published and additionally requires its first publish plus protected environment/tag-policy setup before release. The GitHub environment has no required reviewer or wait timer, and its custom deployment policies currently allow the four published package tag prefixes; adding `proper-compact-v*` is a maintainer prerequisite, not a development side effect. Repository tag rules supply the maintainer-only tag boundary; the release maintainer also needs ruleset bypass for the atomic `main` version-commit push. npm token publishing is disabled after the trusted path is proven.
...
Git uses `.beads/hooks` through `core.hooksPath`. The Beads-managed `pre-commit` and `pre-push` shims chain into `.pre-commit-config.yaml`; commit checks run deterministic local validation, while push checks skip everything the commit gate already ran and add only the offline router smoke, npm audits and signature verification, and OSV dependency scanning. Coverage-thresholded runs (`npm run test:coverage`) stay available per package but are not part of either gate.

Biome enforces formatting, recommended lint rules, JSON syntax and duplicate-key checks, explicit-any rejection in runtime source, and focused or skipped test rejection. The four TypeScript packages enable strict diagnostics, unchecked-index checks, exact optional properties, unused checks, and no-emit compilation. Markdown, shell, YAML, TOML, spelling, file hygiene, package-lock consistency, package contents, and lat.md links are also checked.
